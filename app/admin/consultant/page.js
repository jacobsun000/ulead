...
export default function ConsultantManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!confirm(`确定要${action}该顾问信息吗？`)) return;
  ...
  const handleEdit = (consultantRecord) => {
    setForm({ ...consultantRecord });
  };
  ...
  const handleUpdateOrder = async () => {
    ...
    setLoading(true);
    try {
      const items = consultants.map((item, index) => ({
        id: item.id,
        order_index: index,
      }));
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <h1 className="text-4xl font-bold text-primary mb-6">顾问管理</h1>
          ...
          <div className="bg-white shadow-md rounded-lg overflow-hidden">
            ...
            >
              <table className="table-auto w-full text-left">
                ...
                <SortableContext
                  items={consultants.map((m) => m.id)}
                  strategy={verticalListSortingStrategy}
                >
                  <tbody>
                    {consultants.map((record) => (
